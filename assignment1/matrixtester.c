...
int* multiply_naive(double *a, double *b, double *c, int n) { 
  int i, j, k; 
  for (i = 0; i < n; i++)
      for (j = 0; j < n; j++) 
    	for (k = 0; k < n; k++) 
...
/* Multiply n x n matrices a and b  */ 
bool array_equal(double *a, double *b, int n) { 
  int i, j, k; 
  for (i = 0; i < n; i++)
      for (j = 0; j < n; j++) 
	      if (b[i*n + j] != a[i*n + j]) { return false};
} 


/* Multiply n x n matrices a and b  */ 
int* multiply_blocked(double *a, double *b, double *c, int n, int B) {
  int i, j, k, i1, j1, k1;
  for (i = 0; i < n; i+=B)
    for (j = 0; j < n; j+=B)
      for(k = 0; k < n; k+=B)
...
main ( int arc, char **argv ) {
  ...
  double* c =multiply_blocked(a,b,c,n,block_size);
  } else {
